...
void build_tree(FILE *fp)
{
	...
	while (!feof(fp))
	{
		...
		for (i = 0; i < len; i++)
		{
			if (strcode[i] == '0')
			{
				if (curr->left == NULL) curr->left = talloc();
				curr = curr->left;
			}
			else
			{
				if (curr->right == NULL) curr->right = talloc();
				curr = curr->right;
			}
			// struct tnode **next = strcode[i] == '0' ? &curr->left : &curr->right;
			// if (*next == NULL)
			// {
			// 	*next = talloc();
			// }
...
int main()
{
	const char *IN_FILE = "book_encoded.txt";
	const char *CODE_FILE = "book_code.txt";
	const char *OUT_FILE = "book_decoded.txt";
	FILE *fout;
	FILE *fin;
	/*allocate root*/
	root = talloc();
