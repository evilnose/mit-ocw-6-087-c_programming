...
int main(void) {
    ...
    do {
        ...
        len = strlen(input);
        /* remove trailing newline characte r */
...
int should_clear(p_expr_token op_top, p_expr_token op_input) {
    if (op_top == NULL) return 0;
    int inp_code = op_input->value.op_code;
    int top_code = op_top->value.op_code;
    if (op_top->type == LPARENS) {
        return 0;
    }
    int prec_top = op_precedences[top_code];
    int prec_input = op_precedences[inp_code];
    if (prec_top > prec_input)
        return 1;
    return (prec_top == prec_input && op_associativity[top_code] == LEFT);
}
...
struct token_queue infix_to_postfix(struct token_queue *pqueue_infix) {
    ...
    output.front = output.back = NULL;

    p_expr_token op_top = NULL;

    p_expr_token p;
    while ((p = dequeue(pqueue_infix)) != NULL) {
        if (p->type == OPERAND) {
            enqueue(&output, p);
        } else {
            if (p->type == RPARENS) {
                free(p);
                while (op_top->type != LPARENS) {
                    enqueue(&output, pop(&op_top));
                }
                p = op_top;
                pop(&op_top);
                free(p);
            } else {
                while (should_clear(op_top, p)) {
                    enqueue(&output, pop(&op_top));
                }
                push(&op_top, p);
            }
...
/* postcondition: returned value is final answer, and pqueue_postfix should be empty */
double evaluate_postfix(struct token_queue *pqueue_postfix) {
    p_expr_token s_top = NULL;

    p_expr_token p;
    while ((p = dequeue(pqueue_postfix)) != NULL) {
        if (p->type == OPERAND) {
            push(&s_top, p);
        } else {
            double value;
            if (op_operands[p->value.op_code] == 2) {
                double a = pop(&s_top)->value.operand;
                double b = pop(&s_top)->value.operand;
                switch (p->value.op_code) {
                    ...
                    case DIVIDE:
                        ...
                        break;
                    default:
                        printf("ERROR: Unrecognized operator code\n");
                        value = 0;
                        break;
                }
            } else {
                double n = s_top->value.operand;
                pop(&s_top);
                switch (p->value.op_code) {
                    case NEGATE:
                        value = -n;
            ...
            free(p);
            union token_value token_val;
            token_val.operand = value;
            push(&s_top, new_token(OPERAND, token_val));
        }
    }
    return pop(&s_top)->value.operand;
}
