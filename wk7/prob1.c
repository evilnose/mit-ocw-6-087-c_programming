...
{
	/* find in between */
	int icmp, L = 0, R = pnode->nkeys - 1, M = 0;
	...
	{
		M = (L + R) / 2;
		icmp = key_compare(pnode->keys[M], key);
		if (icmp == 0)
		{
			return -(M + 1);
		}
	...
	if (pnode->nkeys == 0 || key_compare(pnode->keys[M], key) > 0)
		return M;
	return M + 1;
}
...
int initialize_db(const char *filename)
{
	...
	else
	{
		...
		return sqlite3_exec(pDb, sql, store_result, NULL, &errmsg);
	}
	sqlite3_close(pDb);
}
...
{
	nodevalue *val = find_value(title);
	if (val != NULL)
	{
		display_record(val, stdout);
		return 1;
	}
	return 0;
}
...
// 	size_t len;
// 	while (fgets(query, MAX_QUERY_LEN, stdin) != NULL)
// 	{
// 		len = strlen(query);
...
// 		nodevalue *val = find_value(query);
// 		if (val != NULL)
// 		{
// 			display_record(val, stdout);
// 		}
